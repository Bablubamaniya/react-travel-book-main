...
import {getCity,getCities,createCity,updateCity, deleteCity} from "../servieces/apiCities"
...
const reducer = function(cState,action){
    switch(action.type){
        case 'cities/load':{
            ...
            default:
                return new Error(
                    `no Action found with the neme: '${action.type}'`
                );
...
async function handleEditCity(updateCity,id){
    try {
        ...
        dispatch({type:"city/update",payload:{id:id,newCity:data}});
    } catch (error) {
        dispatch({type:"error",payload:error.messege});
    ...
    const valueObj = { cities:cities,loading:loading,error:error,
        handleAddCity:handleAddCity,handleEditCity:handleEditCity,handleRemoveCity:handleRemoveCity,handleLoadCity:handleLoadCity,
     };
    return (
